...
class LauncherBase(DirectObject):
    ...
    def download(self, serverFilePath, localFilename, callback, callbackProgress):
        self.launcherMessage(self.Localizer.LauncherDownloadFile % {'name': self.currentPhaseName,'current': self.currentPhaseIndex,'total': self.numPhases})
        ...
        if self.httpChannel.run():
            ...
            if now - task.lastUpdate >= self.UserUpdateDelay:
                ...
                if totalBytes:
                    ...
                    self.launcherMessage(self.Localizer.LauncherDownloadFilePercent % {'name': self.currentPhaseName,'current': self.currentPhaseIndex,'total': self.numPhases,'percent': pct})
                else:
                    self.launcherMessage(self.Localizer.LauncherDownloadFileBytes % {'name': self.currentPhaseName,'current': self.currentPhaseIndex,'total': self.numPhases,'bytes': bytesWritten})
    ...
    def downloadMultifileTask(self, task):
        ...
        if self.httpChannel.run():
            ...
            if now - task.lastUpdate >= self.UserUpdateDelay:
                ...
                self.launcherMessage(self.Localizer.LauncherDownloadFilePercent % {'name': self.currentPhaseName,'current': self.currentPhaseIndex,'total': self.numPhases,'percent': percentComplete})
            self.foregroundSleep()
            return task.cont
        statusCode = self.httpChannel.getStatusCode()
        statusString = self.httpChannel.getStatusString()
    ...
    def extract(self, mfname, localFilename, destDir, callback):
        ...
        self.launcherMessage(self.Localizer.LauncherExtractingFile % {'name': self.currentPhaseName,'current': self.currentPhaseIndex,'total': self.numPhases})
    ...
    def extractTask(self, task):
        ...
        if errorCode == EUOk:
            ...
            if now - task.lastUpdate >= self.UserUpdateDelay:
                ...
                self.launcherMessage(self.Localizer.LauncherExtractingPercent % {'name': self.currentPhaseName,'current': self.currentPhaseIndex,'total': self.numPhases,'percent': int(round(progress * 100.0))})
            ...
            self.launcherMessage(self.Localizer.LauncherExtractingPercent % {'name': self.currentPhaseName,'current': self.currentPhaseIndex,'total': self.numPhases,'percent': 100})
    ...
    def patch(self, patchFile, patcheeFile, callback):
        ...
        self.launcherMessage(self.Localizer.LauncherPatchingFile % {'name': self.currentPhaseName,'current': self.currentPhaseIndex,'total': self.numPhases})
    ...
    def patchTask(self, task):
        ...
        if errorCode == EUOk:
            ...
            if now - task.lastUpdate >= self.UserUpdateDelay:
                ...
                self.launcherMessage(self.Localizer.LauncherPatchingPercent % {'name': self.currentPhaseName,'current': self.currentPhaseIndex,'total': self.numPhases,'percent': int(round(progress * 100.0))})
            self.foregroundSleep()
            return task.cont
        elif errorCode == EUSuccess:
            self.launcherMessage(self.Localizer.LauncherPatchingPercent % {'name': self.currentPhaseName,'current': self.currentPhaseIndex,'total': self.numPhases,'percent': 100})
    ...
    def getMultifile(self, mfname):
        ...
        if not self.dldb.clientMultifileComplete(mfname) or not self.dldb.clientMultifileDecompressed(mfname):
            ...
            if not self.dldb.clientMultifileComplete(mfname) or not decompressedFilename.exists():
                ...
                localFilename = Filename(self.mfDir, Filename(mfname))
                currentSize = localFilename.exists() or 0
            else:
    ...
    def patchMultifile(self):
        self.launcherMessage(self.Localizer.LauncherCheckUpdates % {'name': self.currentPhaseName,'current': self.currentPhaseIndex,'total': self.numPhases})
