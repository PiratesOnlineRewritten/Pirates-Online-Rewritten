...
class WaterSplash(PooledEffect, EffectController):
    ...
    def createTrack(self, lod=Options.SpecialEffectsHigh):
        self.startEffect = Sequence(Func(self.p0.setBirthRate, 0.015), Func(self.p0.clearToInitial), Func(self.f.start, self, self), Func(self.f.reparentTo, self))
    ...
    def setEffectScale(self, effectScale):
        ...
        self.p0.renderer.setInitialXScale(0.03 * self.cardScale * effectScale)
        ...
        self.p0.renderer.setFinalYScale(0.05 * self.cardScale * effectScale)
