...
class TimeOfDayManager(FSM.FSM, TimeOfDayManagerBase.TimeOfDayManagerBase):
    ...
    def __init__(self):
        ...
        self.isPaused = 0
        self.use_shader = base.config.GetBool('want-shaders', 1) and base.win and base.win.getGsg() and base.win.getGsg().getShaderModel() >= GraphicsStateGuardian.SM20
    ...
    def toggleDebugMode(self):
        if self.debugTOD:
            ...
            base.talkAssistant.receiveGameMessage('Time is %s %s %s' % (str(self.getCurrentIngameTime())[:5], TODGlobals.StateDict[self.currentState], TODGlobals.ENV_DEBUG_NAMES[self.environment]))
    ...
    def startFlutter(self):
        ...
        taskMgr.add(self.__flutterLights, 'todManager-flutterLights')
    ...
    def __flutterLights(self, task):
        ...
        self.frontLightFlutter = self.frontLightFlutter * 0.8 + factorFront * 0.2
        self.backLightFlutter = self.backLightFlutter * 0.8 + factorBack * 0.2
    ...
    def insertTODSetting(self, env, todId, settingName, settingValue):
        ...
        if tod == 'BASE' and env == TODGlobals.ENV_DEFAULT:
            pass
        elif TODGlobals.ENV_SETTINGS_DICT[env][tod].get(settingName):
            del TODGlobals.ENV_SETTINGS_DICT[env][tod][settingName]
    ...
    def addEnvSub(self, baseEnv, envReplacement, netTime=None):
        ...
        if needStartRetro:
            if self.debugTOD:
                base.talkAssistant.receiveGameMessage('Starting time of day for retroActive TimeStamp')
            if self.transitionIval:
    ...
    def getSubTimeList(self, cycleType, env):
        ...
        if sub:
            ...
            if timeStamp > fieldStartFrameTime:
                if timeStamp < fieldEndFrameTime:
            ...
            newBaseCycleEntry = sub or absoluteTimeList
        else:
            envPre = sub[1]
            envPost = sub[2]
            if subInField:
            ...
            for timeEntry in absoluteTimeList:
                ...
                if subAfterField or subInField and endTime < subStart:
                    regEntry = (
                     vId, startTime, transEndTime, endTime, envPre)
                    subListAbsolute.append(regEntry)
                    index += 1
                elif subBeforeField or subInField and startTime > subEnd:
                    regEntry = (
                     vId, startTime, transEndTime, endTime, envPost)
                    subListAbsolute.append(regEntry)
                    index += 1
                elif subEnd > endTime:
                    regEntry = (
                    ...
                    subListAbsolute.append(regEntry)
                    index += 1
                elif subStart < transEndTime:
                    regEntry = (
                     vId, startTime, transEndTime, transEndTime, envPre)
                    index += 1
                    subListAbsolute.append(regEntry)
        ...
        return (
         fieldStartFrameTime, newBaseCycleEntry)
    ...
    def doStartTimeOfDay(self, initial=0):
        ...
        if secondsIntoCycle < 0:
            ...
            taskMgr.remove(self.waitTaskName)
            timeToNextState = pauseTOD or self.currentStateDuration - elapsedTime
        ...
        messenger.send('timeOfDayChange', [self.currentState, stateDuration, elapsedTime, transitionTimeSeconds])
        return (
         stateDuration, elapsedTime, transitionTimeSeconds)
    ...
    def enterIndoors(self, todSettings=None):
        ...
        if todSettings:
            ...
            if todSettings.get('FogTypes') and todSettings['FogTypes'].get(self.currentState):
                ...
                self.setFogType(fogType)
        messenger.send('timeOfDayChange', [self.currentState, 0, 0, 0])
    ...
    def recalcClosestAmbNode(self, task=None):
        ...
        if timeCurrent < timeStart[0] or timeCurrent > timeEnd[1]:
            ...
            self.envSound.setVolume(0.7 * self.timeVol)
            if task:
                return task.done
        return
    ...
    def setSaintPatricksSea(self):
        if self.use_shader:
            self.modifyWaterColor(VBase3(0.22, 0.56, 0.15))
            self.modifyWaterColorFactor(VBase3(0.4, 1.0, 0.3))
        else:
            self.modifyWaterColor(VBase4(0.25, 0.9, 0.2, 1.0))
            self.modifyWaterColorFactor()
